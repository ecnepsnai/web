...
// MimeGetter the implementation used to determine the value of the "Content-Type" header for static files. By default
// this uses a simple implentation that guesses the MIME type based on the extension in the files name.
var MimeGetter IMime = &extensionMimeGetterType{}

type extensionMimeGetterType struct{}

var extensiomMimeMap = map[string]string{
	"aac":    "audio/aac",
...
func (e *extensionMimeGetterType) GetMime(filePath string) string {
	extension := strings.ToLower(filePath[strings.LastIndex(filePath, ".")+1:])
	mime, found := extensiomMimeMap[extension]
	if !found {
